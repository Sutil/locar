...
public class ReservaController extends LocarController{
	...
	private BooleanBuilder queryCliente(String query) {
		BooleanBuilder builder = new BooleanBuilder().or(cliente.nome.containsIgnoreCase(query))
		.or(cliente.cpf.containsIgnoreCase(query));
		return builder;
	}
	...
	private BooleanBuilder queryVeiculo(String query) {
		...
		.or(QVeiculo.veiculo.modelo.containsIgnoreCase(query))
		.or(QVeiculo.veiculo.marca.containsIgnoreCase(query));
		try{
